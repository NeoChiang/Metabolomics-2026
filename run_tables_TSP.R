...
# ** TSP normalisation (no sum normalisation) **
# ============================================================================
...
# metabolite columns = 5 to (ncol - 1), last col is TSP
tsp_col  <- "TSP"
met_cols <- setdiff(colnames(dat)[5:ncol(dat)], tsp_col)
cat("Metabolite columns:", length(met_cols), "\n")

# coerce to numeric
for (mc in c(met_cols, tsp_col)) dat[[mc]] <- suppressWarnings(as.numeric(dat[[mc]]))
cat("TSP range:", range(dat[[tsp_col]], na.rm = TRUE), "\n")
...
# Step 2: TSP normalisation (each sample divided by its own TSP)
data_norm <- data_met / tsp_vec
...
cat("Preprocessing done (TSP normalisation).\n")
...
cat("\n=== Building tables (TSP normalisation) ===\n")
...
# ---- 7. write CSVs --------------------------------------------------------
write.csv(t1, "Table1_6M_GA_TSP.csv",  row.names = FALSE, fileEncoding = "UTF-8")
write.csv(t2, "Table2_2Y_GA_TSP.csv",  row.names = FALSE, fileEncoding = "UTF-8")
write.csv(t3, "Table3_6M_BPD_TSP.csv", row.names = FALSE, fileEncoding = "UTF-8")
write.csv(t4, "Table4_2Y_BPD_TSP.csv", row.names = FALSE, fileEncoding = "UTF-8")
...
saveWorkbook(wb, "Table_summary_TSP.xlsx", overwrite = TRUE)

cat("\nDone. 4 CSVs + Table_summary_TSP.xlsx written.\n")
