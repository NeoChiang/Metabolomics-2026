...
# Step 2: sum normalisation
row_sums <- rowSums(data_met)
data_norm <- data_met / row_sums
...
# ---- 7. write CSVs --------------------------------------------------------
write.csv(t1, "Table1_6M_GA.csv",  row.names = FALSE, fileEncoding = "UTF-8")
write.csv(t2, "Table2_2Y_GA.csv",  row.names = FALSE, fileEncoding = "UTF-8")
write.csv(t3, "Table3_6M_BPD.csv", row.names = FALSE, fileEncoding = "UTF-8")
write.csv(t4, "Table4_2Y_BPD.csv", row.names = FALSE, fileEncoding = "UTF-8")
...
saveWorkbook(wb, "Table_summary.xlsx", overwrite = TRUE)

cat("\nDone. 4 CSVs + Table_summary.xlsx written.\n")
