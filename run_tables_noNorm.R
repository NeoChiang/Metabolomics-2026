...
meta_cols <- 1:4
met_cols  <- colnames(dat)[5:ncol(dat)]
cat("Metabolite columns:", length(met_cols), "\n")

# coerce metabolite columns to numeric
for (mc in met_cols) dat[[mc]] <- suppressWarnings(as.numeric(dat[[mc]]))
...
# Step 2: sum normalisation — SKIPPED

# Step 3: log2 with half-min imputation for zeros
min_nonzero <- min(data_met[data_met > 0], na.rm = TRUE)
data_met[data_met == 0] <- min_nonzero / 2
data_log2 <- log2(data_met)
...
cat("Preprocessing done (no sum normalisation).\n")
...
cat("\n=== Building tables (no sum normalisation) ===\n")
...
# ---- 7. write CSVs --------------------------------------------------------
write.csv(t1, "Table1_6M_GA_noNorm.csv",  row.names = FALSE, fileEncoding = "UTF-8")
write.csv(t2, "Table2_2Y_GA_noNorm.csv",  row.names = FALSE, fileEncoding = "UTF-8")
write.csv(t3, "Table3_6M_BPD_noNorm.csv", row.names = FALSE, fileEncoding = "UTF-8")
write.csv(t4, "Table4_2Y_BPD_noNorm.csv", row.names = FALSE, fileEncoding = "UTF-8")
...
saveWorkbook(wb, "Table_summary_noNorm.xlsx", overwrite = TRUE)

cat("\nDone. 4 CSVs + Table_summary_noNorm.xlsx written.\n")
